...
case "${unameOut}" in
Linux*) export KERL_CONFIGURE_OPTIONS="--enable-smp-support --without-odbc --enable-kernel-poll --without-javac --enable-threads --enable-hipe --with-ssl" ;;
...
function asdf_get_versions() {
  ...
  YARN=$(cat ./.tool-versions | grep yarn | awk '{print $2}')
}
...
function asdf_install_versions() {
  ...
  if [[ "${NODEJS}" != "" ]]; then
    INSTALLED=$(asdf list nodejs | grep "${NODEJS}")
    if [[ "${INSTALLED}" = "" ]]; then
      echo "Installing NodeJS ${NODEJS}"
      asdf install nodejs ${NODEJS}
    else
      echo "NodeJS ${NODEJS} already installed"
    fi
  fi
  if [[ "${YARN}" != "" ]]; then
    INSTALLED=$(asdf list yarn | grep "${YARN}")
    if [[ "${INSTALLED}" = "" ]]; then
      echo "Installing yarn ${YARN}"
      asdf install yarn ${YARN}
    else
      echo "yarn ${YARN} already installed"
    fi
  fi
}
...
echo "Current directory, asdf location, mix version"
